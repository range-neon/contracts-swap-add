...
contract MulticallSwapAddMantle{
    ...
    function swapAddBase(bytes calldata data) external payable {
        ...
        console.log(IERC20Upgradeable(swapData.token0).balanceOf(address(this)));
    ...
    function swapRemoveBase(bytes memory data) external {
        ...
        if (swapData.zeroForOne) {
            ...
            //4. swap to WMNT
            Address.functionCall(swapData.target, swapData.callData);
            wrapped.withdraw(IERC20Upgradeable(swapData.token0).balanceOf(address(this)));
