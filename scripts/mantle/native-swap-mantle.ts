...
async function main() {
    ...
    const multicallAddress= '0x631Fc65b18Ac3Aa89D757e60bc27e51E3b8d18C6'

    const response = await fetch(`https://newapi.native.org/v1/firm-quote?chain=mantle&token_in=${WMNT}&token_out=${WETH}&amount=1.66&from_address=${multicallAddress}&slippage=20&excluded_sources=openOcean`,
    ...
    const swap = await multicallCtx.swapAddBase(encodedData, { value: ethers.utils.parseEther("3.32"), gasLimit: 5000000 })
    console.log(swap)
    ...
    // const swapRemoveUrl = `https://newapi.native.org/v1/firm-quote?chain=arbitrum&token_in=${WETH}&token_out=${WMNT}&amount=${parseAmount}&from_address=${multicallAddress}&slippage=20&excluded_sources=openOcean`
